...
public class OptionalInt {
     ...
     * {@code null}
     */
    public void ifPresent(Consumer<Integer> action) throws Exception {
        if (isPresent) {
            action.accept(value);
        }
    }
     ...
     * @since 9
     */
    public void ifPresentOrElse(Consumer<Integer> action, Runnable emptyAction) throws Exception {
        if (isPresent) {
            action.accept(value);
        } else {
            emptyAction.run();
     ...
     */
    public int orElseGet(Supplier<Integer> supplier) {
